...
- `عدد_معرفات_التسلسل` / `nSeqId` (`سند[مصفوفة[صحيح]]` / `ref[array[Int]]`) - عدد معرفات التسلسل لكل رمز. يقابل `n_seq_id`.
- `معرفات_التسلسل` / `seqId` (`سند[مصفوفة[سند[مصفوفة[مـعرف_تسلسل]]]]` / `ref[array[ref[array[SeqId]]]]`) - معرفات التسلسل. يقابل `seq_id`.
- `الخرج` / `output` (`سند[مصفوفة[صحيح[8]]]` / `ref[array[Int[8]]]`) - أعلام الإخراج. يقابل `logits`.
...
```alusus
اشمل "مـحا"؛
مـحا.اشمل_حزمة("Alusus/Llama@0.2")؛
استخدم لـاما؛ 
...
// 2. تحميل النموذج
عرف معطيات_النموذج: نـموذج.مـعطيات = نـموذج.هات_معطيات_افتراضية()؛
معطيات_النموذج.عدد_طبقات_المعالج_الرسومي = 0؛  // المعالج المركزي فقط
عرف نموذج: سند[نـموذج](نـموذج.حمل("model.gguf"، معطيات_النموذج))؛
...
// 5. الترميز وفك ترميز المحث
عرف رموز: مصفوفة[رمـز، 512]؛
عرف عدد_الرموز: صـحيح = رمز(نموذج.المفردات، "Hello world"، 11، رموز، 512، 1، 1)؛
عرف دفعة: دفـعة = دفـعة.هات_واحدا(رموز، عدد_الرموز)؛
سياق.فك_ترميز(دفعة)؛
...
بينما صح {    
    ...
    إذا نموذج.المفردات.أهي_نهاية_التوليد(معرف) اقطع؛ 

    // تحويل الرمز إلى نص وطباعته
    عرف صوان: مصفوفة[محـرف، 64]؛ Char
    فك_ترميز(نموذج.المفردات، معرف~مؤشر~تحويل[سند[مصفوفة[رمـز]]]، 1، صوان~مؤشر، 64، 0، 0)؛
    // ... طباعة صوان

    // فك ترميز الرمز التالي
    عرف الدفعة_التالية: دفـعة = دفـعة.هات_واحدا(معرف~مؤشر~تحويل[سند[مصفوفة[رمـز]]]، 1)؛
    سياق.فك_ترميز(الدفعة_التالية)؛
}
